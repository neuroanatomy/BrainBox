...
describe('TESTING WEBSOCKET WORKFLOW', function () {
    ...
    describe('WS connection', function () {
        ...
        it('Can trigger a data download', async function () {
            ...
            let res = await chai.request(U.serverURL).get('/mri/json').query({url: U.localBertURL});
            assert(res.statusCode === 404, "Unexpected status code");

            // trigger download
            res = await chai.request(U.serverURL).post('/mri/json').send({url: U.localBertURL,token: U.testToken + U.userFoo.nickname});
            let {body} = res;
