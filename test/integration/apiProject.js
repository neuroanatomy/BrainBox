...
describe('/project route', () => {
  ...
  it('get("/project/json/") should get an error message requesting page', async () => {
    const res = await chai.request(url).get('/project/json/');
    const {body} = res;
  ...
  it('get("/project/json?page=0") should return an array', async () => {
    const res = await chai.request(url).get('/project/json?page=0')
        .query({page: 0});
    const {body} = res;
  ...
  it('get("/project/json/test") should return status 200', async () => {
    const res = await chai.request(url).get('/project/json/test');
    assert.equal(res.statusCode, 200);
  });
