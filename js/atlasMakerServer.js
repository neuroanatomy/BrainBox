...
const whitelist = JSON.parse(fs.readFileSync(__dirname + "/../whitelist.json"));
...
*/
    ...
    var numberOfUsersConnectedToMRI = function numberOfUsersConnectedToMRI(path) {
        ...
        for(var i in US) {
            ...
            if(typeof US[i].User.mri === 'undefined') {
    ...
    var displayAtlases = function displayAtlases() {
        ...
        var i;
        console.log("\n" + Atlases.filter(function(o){return typeof o !== 'undefined'; }).length + " Atlases:");
        for(i in Atlases) {
    ...
    var displayBrains = function displayBrains() {
        ...
        console.log("\n" + this.Brains.filter(function(o){return typeof o !== 'undefined'; }).length + " Brains:");
        var i;
        for(i in this.Brains) {
            var sum = numberOfUsersConnectedToMRI(this.Brains[i].path);
            console.log("Brains[" + i + "].path=" + this.Brains[i].path + ", " + sum + " users connected");
        }
        for(i in this.Brains) {
            console.log("Brains[" + i + "]");
            console.log("           path:", this.Brains[i].path);
            console.log("       data.dim:", this.Brains[i].data.dim);
            console.log("    data.pixdim:", this.Brains[i].data.pixdim);
            console.log("data.vox_offset:", this.Brains[i].data.vox_offset);
            console.log("       data.dir:", this.Brains[i].data.dir);
            console.log("       data.ori:", this.Brains[i].data.ori);
            console.log("       data.s2v:", this.Brains[i].data.s2v);
            console.log("       data.v2w:", this.Brains[i].data.v2w);
            console.log("      data.wori:", this.Brains[i].data.wori);
            console.log("  data.datatype:", this.Brains[i].data.datatype);
            console.log("       data.sum:", this.Brains[i].data.sum);
            console.log();
        }
    };
    var displayUsers = function displayUsers() {
        traceLog(displayUsers);
        console.log("\n" + US.filter(function(o){return typeof o !== 'undefined'; }).length + " User Sockets:");
        for(var i in US) {
...
*/
    ...
    var broadcastPaintVolumeMessage = function broadcastPaintVolumeMessage(msg, User) {
        ...
        try {
            var n = 0, i, msg2 = JSON.stringify({"type": "paintvol", "data": msg});
            for(i in US) {
                if( typeof US[i].User !== 'undefined' &&
                    US[i].User.iAtlas!=User.iAtlas ) {
                    continue;
                }
                US[i].socket.send(msg2);
                n += 1;
            }
            if(debug) {
...
//        var dim = atlas.dim;
        ...
        switch(view) {
            case 'sag': brainWidth = sdim[1]; brainHeight = sdim[2]; break; // sagital
            case 'cor': brainWidth = sdim[0]; brainHeight = sdim[2]; break; // coronal
            case 'axi': brainWidth = sdim[0]; brainHeight = sdim[1]; break; // axial
        }
...
//        var dim = Atlases[User.iAtlas].dim;
        ...
        while(Q.length>0) {
            ...
            while (right + 1<brainWidth && vol[sliceXYZ2index(right + 1, y, z, User)] == bval) {
                right += 1;
            }
            for(x = left; x<=right; x += 1) {
                ...
                if(y + 1<brainHeight && vol[sliceXYZ2index(x, y + 1, z, User)] == bval) {
                    Q.push({x: x, y: y + 1});
                }
            }
        }
    ...
    */
    ...
    var computeS2VTransformation = function computeS2VTransformation(mri) {
        ...
        */
        ...
        for(j in mri.dir) {
            for(i in mri.dir[j]) {
                v2w[i][j] = mri.dir[i][j]; // do not transpose
            }
        }
...
*/
    ...
    var readMGZ = function readMGZ(path) {
        ...
        */
        ...
        var pr = new Promise(function (resolve, reject) {
            try {
                childProcess.execFile("gunzip", ["-c", path], {encoding: 'binary', maxBuffer: 200*1024*1024}, function(err, stdout) {
                    var mgh = new Buffer(stdout, 'binary');
                    var i, j, tmp, sum, mri = {};
                    var sz, bpv;
                    var hdrSize = 284, ftrSz;
                    MghHdr.allocate();
                    MghHdr._setBuff(mgh);
                    ...
                    var Pxyz0 = subVecVec(h.Pxyz_c, mulMatVec(MdcD, PcrsC));
                    var M = [
    ...
    var getBrainAtPath = function getBrainAtPath(brainPath) {
        ...
        */
        var i;
        for(i in this.Brains) {
            if(this.Brains[i].path === brainPath) {
                if(debug>1) {
                    console.log("    brain already loaded");
                }
                return Promise.resolve(this.Brains[i].data);
            }
        }
    ...
    */
    var drawSlice = function drawSlice(brain, view, slice) {
        ...
        var x, y, i, j;
        var brainWidth, brainHeight; //, brain_D;
        var ys, ya, yc;
        var val;
        var s, s2v = brain.s2v;

        switch(view) {
            case 'sag': brainWidth = s2v.sdim[1]; brainHeight = s2v.sdim[2]; break; //brain_D = s2v.sdim[0]; break; // sagital
            case 'cor': brainWidth = s2v.sdim[0]; brainHeight = s2v.sdim[2]; break; //brain_D = s2v.sdim[1]; break; // coronal
            case 'axi': brainWidth = s2v.sdim[0]; brainHeight = s2v.sdim[1]; break; //brain_D = s2v.sdim[2]; break; // axial
        }
    ...
    var sendSliceToUser = function sendSliceToUser(brain, view, slice, userSocket) {
    ...
    var receiveRequestSliceMessage = function receiveRequestSliceMessage(data, userSocket) {
        ...
        getBrainAtPath(brainPath)
            .then(function promiseFromReceiveRequestSliceMessage(data) {
                sendSliceToUser(data, view, slice, userSocket);
            });
    };

    var receiveRequestSlice2Message = function receiveRequestSlice2Message(data, userSocket) {
    ...
    var receiveSaveMessage = function receiveSaveMessage(data /*, userSocket*/) {
...
//        var modifiedBy = (sourceUS.User && sourceUS.User.username) ? sourceUS.User.username : "anonymous";
    ...
    var receiveSaveMetadataMessage = function receiveSaveMetadataMessage(data/*, userSocket*/) {
    ...
    var receiveAtlasFromUserMessage = function receiveAtlasFromUserMessage(data/*, userSocket*/) {
    ...
    var unloadUnusedBrains = function unloadUnusedBrains() {
        ...
        for(i in this.Brains) {
            ...
            if(sum === 0) {
                console.log("    No user connected to MRI " + this.Brains[i].path + ": unloading it");
                unloadMRI(this.Brains[i].path);
            }
        }
    };
    ...
    var sendAtlasToUser = function sendAtlasToUser(atlasdata, userSocket, flagCompress) {
        ...
        if(flagCompress) {
            ...
            zlib.gzip(atlasdata, function (err, atlasdatagz) {
                ...
                try {
                    userSocket.send(Buffer.concat([atlasdatagz, niiTag]), {binary: true, mask: false});
                } catch(e) {
            ...
            try {
                userSocket.send(Buffer.concat([atlasdata, niiTag]), {binary: true, mask: false});
            } catch(e) {
    ...
    */
    ...
    var receiveUserDataMessage = function receiveUserDataMessage(data, userSocket) {
            ...
            if(data.description === "sendAtlas") {
                ...
                if(atlasLoadedFlag) {
                    if(firstConnectionFlag || switchingAtlasFlag) {
                        ...
                        sendAtlasToUser(Atlases[i].data, userSocket, true);
                    ...
                    addAtlas(User)
                        .then(function(atlas) {
                            sendAtlasToUser(atlas.data, userSocket, true);
                            sourceUS.User.isMRILoaded = true;
                        });
                }
            } else {
    ...
    */
    ...
    var declareAutocompleteClient = function declareAutocompleteClient(data/*, userSocket*/) {
    ...
    var initSocketConnection = function initSocketConnection() {
        ...
        try {
            ...
            websocket.on("connection", function connectionFromInitSocketConnection(s) {
                traceLog(connectionFromInitSocketConnection);
                ...
                US.push(newUS);
                console.log("    User id " + newUS.uid + " connected, total: " + US.filter(function(o){return typeof o !== 'undefined'; }).length + " users");

                // send data from previous users
                sendPreviousUserDataMessage(newUS);

                s.on('message', function messageFromInitSocketConnection(msg) {
                    traceLog(messageFromInitSocketConnection, 1);
                    var sender = this;
                    ...
                    for(var i in websocket.clients) {
                        ...
                        if( typeof sourceUS.User === 'undefined' || typeof targetUS.User === 'undefined') {
                            if(debug) {
                                console.log("    User " + sourceUS.uid + ": " + (typeof sourceUS.User === 'undefined')?"undefined": "defined");
                            }
                            if(debug) {
                                console.log("    User " + targetUS.uid + ": " + (typeof targetUS.User === 'undefined')?"undefined": "defined");
                            }
                            continue;
                        }
                ...
                s.on('close', function closeFromInitSocketConnection() {
                    traceLog(closeFromInitSocketConnection);

                    var sum, sourceUS;

                    console.log("A user is disconnecting");
                    console.log("There are " + US.filter(function(o){return typeof o !== 'undefined'; }).length + " connected");
                    ...
                    console.log("    " + US.filter(function(o){return typeof o !== 'undefined'; }).length + " remain connected");
                    console.log();
