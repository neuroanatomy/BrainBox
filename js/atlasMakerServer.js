...
*/
        ...
        displayAtlases: function displayAtlases() {
            ...
            for(i in me.Atlases) {
                ...
                console.log("Atlases[" + i + "] path:" + me.Atlases[i].dirname + me.Atlases[i].name + ", " + sum + " users connected");
            }
            for(i in me.Atlases) {
                console.log("atlas", i, me.Atlases[i]);
            }
        },
        displayBrains: function displayBrains() {
            ...
            for(i in me.Brains) {
                ...
                console.log("Brains[" + i + "].path=" + me.Brains[i].path + ", " + sum + " users connected");
            }
            for(i in me.Brains) {
                console.log("Brains[" + i + "]");
                console.log("           path:", me.Brains[i].path);
                console.log("       data.dim:", me.Brains[i].data.dim);
                console.log("    data.pixdim:", me.Brains[i].data.pixdim);
                console.log("data.vox_offset:", me.Brains[i].data.vox_offset);
                console.log("       data.dir:", me.Brains[i].data.dir);
                console.log("       data.ori:", me.Brains[i].data.ori);
                console.log("       data.s2v:", me.Brains[i].data.s2v);
                console.log("       data.v2w:", me.Brains[i].data.v2w);
                console.log("      data.wori:", me.Brains[i].data.wori);
                console.log("  data.datatype:", me.Brains[i].data.datatype);
                console.log("       data.sum:", me.Brains[i].data.sum);
                console.log();
            }
        },
        displayUsers: function displayUsers() {
            ...
            for(var i in me.US) {
                console.log("US[" + i + "].uid=", me.US[i].uid);
                console.log("US[" + i + "]=", me.US[i].User);
            }
        },
        toggleWebsocketRecording: function toggleWebsocketRecording() {
            ...
            if(me.recordWS) {
                ...
                console.log(JSON.stringify(me.recordedWSTraffic));
    ...
    */
        ...
        */
        ...
        pushUndoLayer: function pushUndoLayer(User) {
            ...
            if(me.debug) {
                console.log("    Number of layers: " + me.UndoStack.length);
            }
        ...
        undo: function undo(User) {
            ...
            */
            ...
            for(j in undoLayer.actions) {
                ...
                if(me.debug>=3) {
                    console.log("    Undo:", i%User.dim[0], parseInt(i/User.dim[0], 10)%User.dim[1], parseInt(i/User.dim[0]/User.dim[1], 10)%User.dim[2]);
                }
            }
            msg = { "data": arr };
            ...
            if(me.debug) {
                console.log("    " + me.UndoStack.length + " undo layers remaining (all users)");
            }
        },
                ...
                try {
                    ...
                    zlib.gunzip(niigz, function (err, nii) {
                        ...
                        try {
                            ...
                            console.log("ERROR Cannot read nifti header:", err2);
    ...
    //        var modifiedBy = (sourceUS.User && sourceUS.User.username) ? sourceUS.User.username : "anonymous";
        ...
        unloadUnusedBrains: function unloadUnusedBrains() {
            ...
            for(i in me.Brains) {
                ...
                if(sum === 0) {
                    console.log("    No user connected to MRI " + me.Brains[i].path + ": unloading it");
        ...
        unloadUnusedAtlases: function unloadUnusedAtlases() {
            ...
            for(i in me.Atlases) {
                var sum = me.numberOfUsersConnectedToAtlas(me.Atlases[i].dirname, me.Atlases[i].name);
                if(sum === 0) {
                    console.log("    No user connected to Atlas " + me.Atlases[i].dirname + me.Atlases[i].name + ": unloading it");
        ...
        loadAtlas: function loadAtlas(User) {
            ...
            var pr = new Promise(function promiseFromloadAtlas(resolve, reject) {
                ...
                if(!fs.existsSync(mriPath)) {
                    ...
                    console.log("    Atlas " + mriPath + " does not exists. Create a new one");
            ...
            process.stdin.on('keypress', function (ch, key) {
                if(key) {
                    ...
                    if(key.name === 'escape') {
                        ...
                        console.log("enterCommands: " + me.enterCommands);
                    }
                ...
                if(ch) {
                    if(me.enterCommands) {
                        switch (ch) {
                            ...
                            case '0':
                                me.debug = 0;
                                console.log("debug level:", me.debug);
                                break;
                            case '1':
                                me.debug = 1;
                                console.log("debug level:", me.debug);
            ...
            */
            try {
                ...
                websocket.on("connection", function connectionFromInitSocketConnection(s) {
                    ...
                    me.US.push(newUS);
                    console.log("    User id " + newUS.uid + " connected, total: " + me.US.filter(function(o) { return typeof o !== 'undefined'; }).length + " users");
                    ...
                    s.on('close', function closeFromInitSocketConnection() {
                        ...
                        console.log("There are " + me.US.filter(function(o) { return typeof o !== 'undefined'; }).length + " connected");
                        ...
                        console.log("    " + me.US.filter(function(o) { return typeof o !== 'undefined'; }).length + " remain connected");
                        console.log();
